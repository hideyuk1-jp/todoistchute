...
$(async function () {
  ...
  $(document).on('click', '#tc-taskbar .task', function () {
    ...
    $('html,body').scrollTop(
      $(`.task_list_item[data-item-id="${$(this).data('target')}"`).offset().top - $('#top_bar').height()
    );
  });
  ...
  function calcTime() {
    ...
    if (location.search.match(/print_mode=1/) || location.hash.match(/activity/) || $('.upcoming_view').length > 0) {
      $('#tc-wrapper').remove();
    ...
    // 開始時刻の入力があればdateを更新
    if ($('#tc-begintime').val() != '' && $('#tc-cbbegintime').prop('checked')) {
    ...
    // 完了予定を更新
    // 休憩時間の入力があれば加算
    if ($('#tc-breaktime').val() != '' && $('#tc-cbbreaktime').prop('checked')) {
  ...
  function getTaskList() {
    ...
    $(`${tcParentId} ul.items`).each((i, els) => {
      ...
      $(els)
        .find('li.task_list_item:not(:hidden, .task_list_item--completed, .moreItemsHint)')
        .each((i2, el) => {
  ...
  function taskTimeSum(el) {
    ...
    $(el)
      ...
      .each((_, label) => {
        const match = $(label)
          .text()
          .match(/^\/\/((?<hours>\d+(\.\d+)?)(:|h|時間))?((?<minutes>\d+)(m|分)?)?$/);
        if (!match) return;
    ...
    const matches = $(el)
      .find('.task_content')
      .text()
      .matchAll(/\/\/((?<hours>\d+(\.\d+)?)(:|h|時間))?((?<minutes>\d+)(m|分)?)?/g);
  ...
  function getStrFromDate(str) {
    ...
    if (str === formatDate(today, 'yyyy-MM-dd')) return chrome.i18n.getMessage('today');
    if (str === formatDate(yesterday, 'yyyy-MM-dd')) return chrome.i18n.getMessage('yesterday');
    if (str === formatDate(tomorrow, 'yyyy-MM-dd')) return chrome.i18n.getMessage('tomorrow');
    return str;
  }
